...
Deno.serve(async (req) => {
  try {
    ...
    const created = await base44.asServiceRole.entities.Newsletter.create(testData);
    console.log("Create response:", JSON.stringify(created));

    // Now try to read it back
    console.log("Reading back by ID...");
    const allNewsletters = await base44.asServiceRole.entities.Newsletter.list('-created_date', 5);
